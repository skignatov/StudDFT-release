...
2. Откройте созданную папку и запустите `set_user_environment.bat`, чтобы настроить переменные среды PATH и STUDDFT для текущего пользователя.
...
3. При необходимости откройте `sd.bat` в любом текстовом редакторе ASCII (Far, Notepad, WordPad и т.д.) и отредактируйте переменные OpenMP 
   ...
   set OMP_NUM_THREADS=4 <-- установите количество ядер на вашем процессоре (1, 4, 8,...). По умолчанию установлено значение 4.

   set OMP_STACKSIZE=64M <-- установите разумный размер стека OpenMP (32M, 64M, 128M, ...)

5. Перейдите в свой рабочий каталог (с вашим входным файлом, например, `h2o.inp`) и запустите программу:
